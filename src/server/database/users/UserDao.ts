...
import { LoginUserData, RegisterUserData, UserRecord, UserData, UserId } from "../../../model/user/UserData";
import { ValidateUserData } from "./utils/ValidateUserData";
import { AuthenticationReturnMsg, AuthenticationSuccess } from "../../authentication/UserValidator";

export const email_or_userid_taken_string: string = "Email or user id already taken.";
...
export class UserDao extends DAO implements IUserDao {
  ...
  constructor(
    ...
    this.insert_user = this.db.prepare(`INSERT INTO ${this.table_name} (user_id, password, salt) VALUES (?, ?, ?);`);

    this.get_user = this.db.prepare(`SELECT * FROM ${this.table_name} WHERE user_id = ?;`);

    this.update_password = this.db.prepare(`UPDATE ${this.table_name} SET password = ?, salt = ? WHERE user_id = ?`);

    this.delete_user_statement = this.db.prepare(`DELETE FROM ${this.table_name} WHERE user_id = ?`);
  }
  ...
  public register_user(data: RegisterUserData): AuthenticationReturnMsg {
    ...
    let hash_and_salt: HashAndSalt = PasswordService.hash_password(data.password);

    let insert_results = this.catch_database_errors_run(
      () => {
        this.insert_user.run(data.user_id, hash_and_salt.hash, hash_and_salt.salt);
        return { success: true };
      },
      new Map([
  ...
  public change_user_password(new_password: string, user_id: UserId): ReturnMsg {
    return this.catch_database_errors_run(() => {
      ...
      let password_validation: ReturnMsg = ValidateUserData.validate_password(new_password);
