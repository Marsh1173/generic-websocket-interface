...
/**
 * This is a variation on the StateObservable that enables observers to also watch specific values of a state in a type-safe way.
 * Unused because state observing implies getting fully updated whenever the state changes even slightly.
 */

const update_prepend_str: string = "update_";
type update_prepend = "update_";

type StateValueObserver<StateType> = Partial<{
...
class StateValueObservable<StateType extends Record<string, any>> {
  protected readonly observer_map: Map<Id, StateValueObserver<StateType>> =
    new Map();
  ...
  public add_observer(observer: StateValueObserver<StateType>): StateType {
