...
const STANDARD_PX_PER_UNIT: number = 100;
export const PixelsPerUnit: Record<Resolution, StaticVector> = {
  "4k": {
    x:
      (ResolutionDimensions["4k"].w / ResolutionDimensions["standard"].w) *
      STANDARD_PX_PER_UNIT,
    y:
      (ResolutionDimensions["4k"].h / ResolutionDimensions["standard"].h) *
      STANDARD_PX_PER_UNIT,
  },
  standard: {
  ...
  mini: {
    x:
      (ResolutionDimensions["mini"].w / ResolutionDimensions["standard"].w) *
      STANDARD_PX_PER_UNIT,
    y:
      (ResolutionDimensions["mini"].h / ResolutionDimensions["standard"].h) *
      STANDARD_PX_PER_UNIT,
  },
};
